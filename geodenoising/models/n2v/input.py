...
def manipulate(
    image: Tensor, subpatch_size: int = 5, inplace: bool = False, perc_pix=1.5
):
    """
    ...
    Args:
        ...
        perc_pix: perctage of pixels to manipulate
    ...
    coords = get_stratified_coords((image_x, image_y), perc_pix=perc_pix)
    ...
    return image, torch.stack([torch.tensor(c) for c in mask_coords])
...
def get_stratified_coords(shape, perc_pix):
    """
    Args:
    ...
    """
    box_size = np.round(np.sqrt(100 / perc_pix)).astype(int)
